...
const allowedGeoSources = new Set(["none", "live", "mock", "exif", "manual"]);
...
export async function POST(request: Request) {
  ...
  if (!bytes.length || bytes.length > 15 * 1024 * 1024) {
    return NextResponse.json({ error: "La foto supera el límite de 15 MB." }, { status: 400 });
  }
  ...
  try {
    ...
    if (uploaded.error && !uploaded.error.message.toLocaleLowerCase().includes("already exists")) {
      throw uploaded.error;
    }
    const createdObject = !uploaded.error;

    const activityId = body.photo.activityId
      ? stableUuid(session.trip.id, "activity", body.photo.activityId)
      : null;
    const stampId = body.photo.achievementId
      ? stableUuid(session.trip.id, "stamp", body.photo.achievementId)
      : null;
    ...
    const source =
      body.photo.geolocationSource && allowedGeoSources.has(body.photo.geolocationSource)
        ? body.photo.geolocationSource
        : body.photo.location
          ? "live"
          : "none";
    const capturedAt = body.photo.capturedAt ?? body.photo.createdAt;
    ...
    const inserted = await client.from("photos").upsert({
      ...
      stamp_id: stampId,
      storage_path: path,
      note: body.photo.note ?? null,
      taken_at: body.photo.createdAt,
      captured_at: capturedAt,
      latitude: body.photo.location?.latitude ?? null,
      longitude: body.photo.location?.longitude ?? null,
      accuracy_m: body.photo.location?.accuracy ?? null,
      geolocation_source: source,
      timezone: body.photo.timezone ?? null,
      local_date: body.photo.localDate ?? null,
      local_time: body.photo.localTime ?? null,
      place_label: body.photo.placeLabel ?? null,
      exif_metadata: body.photo.exifMetadata ?? {},
      is_test: body.photo.isTest ?? false,
    }, { onConflict: "id" });
